...
public class MonthlyInventoryReport extends javax.swing.JFrame {
    ...
    void showTableContent(){
        ...
        try{
            ...
            String query="select pname,sum(quantity)as quantity,month(date) as month  from sellProducts where month(date)=month(curdate()) group by pname;";
            ResultSet rs =st.executeQuery(query);
            while(rs.next()){
                
                p[1]=rs.getString("pname");
                p[3]=rs.getString("quantity");
                p[4]=rs.getString("month");
    ...
    private void initComponents() {
        ...
        dailyPayment.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Product Id", "Product Name", "SupplierId", "Monthly Sold", "Month"
            }
        ) {
        ...
        jPanel1.add(month, new org.netbeans.lib.awtextra.AbsoluteConstraints(160, 140, 180, -1));
    ...
    private void searchBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchBtnActionPerformed
        ...
        try{
            if(!month.getText().equals("")){
                ...
                Statement st=con.createStatement();
                Statement st1=con.createStatement();
                String query="select pname,sum(quantity)as quantity,month(date) as month from sellProducts where month(date)="+month.getText()+" group by pname;";
                ResultSet rs =st.executeQuery(query);
                while(rs.next()){
